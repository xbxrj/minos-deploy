...
import xbp.core.exception.expimp.ExpimpException;
import xbp.core.util.data.Maps;
import xbp.core.util.data.StringUtil;
import xbp.core.util.excel.CascadeHeight;
import xbp.core.util.excel.CascadeHiddenDataInfo;
import xbp.core.util.excel.CascadeInfo;
import xbp.core.util.excel.HiddenDataInfo;
import xbp.core.util.time.TimeUtil;
import xbp.core.util.xml.XML;
import xbp.core.util.xml.XMLElement;
...
public class ExcelUtil {
    ...
    public static Map<String, Object> readTemplateXml(String fileName) throws Exception {
        ...
        try {
            ...
            Thrower.throwException(BaseCommonException.XMLFARMAT_OR_XMLPATH);
        }
                        ...
                        if (ExpImpConstants.EXCEL_TYPE_2.equals(colList.get(colNum).get("type")) && "name".equals(colList.get(colNum).get("class"))) {
                            ...
                            if (sheetCreate) {
                                ...
                                for (String key : hiddenData.keySet()) {
                                    ...
                                    if (size == 0) {
                                        Thrower.throwException(BaseCommonException.NOTFOUND_HIDDEN_DATA, colList.get(colNum).get("value"));
                                    }
    ...
    public Map<String, List<Map<String, String>>> readXml(String fileName) throws Exception {
        ...
        try {
            ...
            Thrower.throwException(BaseCommonException.XML_EXCEPTION, "路径");
    ...
    public static List<Map<String, Object>> insertExcelData(String path, String fileName) throws Exception {
        ...
        if (sheetIndex == -1) {
            Thrower.throwException(BaseCommonException.EXCEPTION_DATA, "excel工作簿中找不到表名为：" + listThead.get(0).get("name") + "的工作表");
        }
        ...
        if (lastRowNum > 0) {
            ...
            if (sheet.getRow(0).getLastCellNum() != listCol.size()) {
                Thrower.throwException(BaseCommonException.EXCEPTION_DATA, "excel模板不是最新版，请下载最新版excel模板后倒入数据！");
